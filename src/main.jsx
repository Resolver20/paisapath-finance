...
class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.failed) return <Screen><p className="eyebrow">NIVARA</p><h1>Your dashboard needs a refresh.</h1><p className="lead">Please reload this page. Your private Firestore data has not been changed.</p><button className="primary" onClick={() => window.location.reload()}>Reload Nivara</button></Screen>;
...
function App() {
  ...
  if (busy) return <Screen><p className="eyebrow">NIVARA</p><h1>Loading your private space…</h1></Screen>;
...
function SignIn({ error }) { return <Screen><span className="brand"><b>N</b> Nivara</span><p className="eyebrow">PRIVATE FINANCE</p><h1>Money, held with calm.</h1><p className="lead">Your details are visible only after you sign in with your approved Google account.</p><button className="primary" onClick={() => signInWithPopup(auth, googleProvider).catch(() => {})}><span>G</span> Continue with Google</button>{error && <p className="error">{error}</p>}<p className="fine">Your financial data lives in your protected Firestore account — never in this website’s source code.</p></Screen>; }
